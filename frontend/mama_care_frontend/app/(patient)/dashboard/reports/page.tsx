...
import { simulateReportUpload, ReportInterpretation } from "@/lib/dashboard-data";
...
export default function ReportsPage() {
  ...
  const handleProcess = async () => {
    ...
    const res = await simulateReportUpload(file, language);
    setResult(res);
    setView("result");
  };
  ...
  const toggleSpeech = async () => {
    ...
    const textToRead = `
      ...
      ${result.abnormalValues.length > 0 ? "Abnormal findings: " + result.abnormalValues.map(v => v.finding + ". " + v.meaning).join(" ") : ""}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #F3F4F6", paddingBottom: "1rem" }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                // If we already have a result, we should re-process to get it in the new language
                if (view === "result") {
                  setView("empty");
                  setResult(null);
                }
